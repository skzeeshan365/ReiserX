...
public class uploadAudioFiles {
    Context context;

    public uploadAudioFiles(Context context) {
        this.context = context;
    }

    public void uploadAudioFiles(FirebaseDatabase mdb, FirebaseStorage storage, String folder, String userID, String fileName, File filePath, int prog) {
        try {
        ...
        if (save.getString(sharedPath+fileName, "").equals("")) {
            ...
            StorageReference reference = storage.getReference("Main").child(userID).child(folder).child("Audios").child(String.valueOf(number));
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            reference.getDownloadUrl().addOnSuccessListener(uri -> {
                                ...
                                mdb.getReference("Main").child(userID).child("Upload").child("Audios").child(values).child(String.valueOf(number))
                                        .setValue(fileUpload)
        ...
        mdb.getReference("Main").child(userID).child("Upload").child("Audios").child("Download progress").setValue(prog);
        } catch(Exception e) {
